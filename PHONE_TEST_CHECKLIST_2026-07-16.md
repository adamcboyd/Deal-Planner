...
Use `show-current-status.ps1` when context is lost or before connecting the phone. It prints the current branch/commit, debug APK source identity, Gemini readiness without printing secrets, ADB visibility, latest report/sample paths, and the exact normal/AI phone-test commands. It is read-only and does not build, install, call the network, or call Gemini.
