...
`new-feature-readiness-report.ps1` creates an ignored timestamped readiness report that separates local source/test evidence from phone-only checks for each input path and AI feature. Run it after `.\gradlew.bat testDebugUnitTest assembleDebug lintDebug` when you need a current recovery snapshot.
...
- `scripts\new-feature-readiness-report.ps1` creates an ignored timestamped feature readiness matrix under `phone-test-results\`, showing which pantry, barcode, flyer, receipt, budget, shopping, Settings, and AI paths have local source/test evidence and which still require Android phone or real Gemini-key verification.
