...
9. Subtotal, tax, total, payment, card tender, EBT/card, coupon, discount, savings, reward, and refund lines are ignored so only grocery purchase items affect spending.
...
- Gemini configuration guardrails and pantry response parsing (placeholder keys, model fallback, whitespace/prefix normalization, fenced JSON, scalar warnings/questions, snake_case/name aliases, malformed string/list fields)
...
- Receipt reconciliation ignores subtotal, tax, total, savings, coupon, discount, reward, refund, and payment/card-tender lines.
...
- Gemini pantry response parsing handles fenced JSON, minor surrounding text, scalar warnings/questions, snake_case/name aliases, malformed string/list fields, and confidence clamping.
