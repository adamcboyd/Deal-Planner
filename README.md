...
`show-current-status.ps1` is the quick lost-context checkpoint. It is read-only and prints the current repo/branch/commit, debug APK source identity, Gemini key/model readiness without printing secrets, ADB phone visibility, latest report/sample paths, and the next phone-test commands. It does not build, install, call the network, or call Gemini.
