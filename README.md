...
- `scripts\start-phone-test-run.ps1` builds the current debug APK first unless `-SkipBuild` is used, can wait for one authorized ADB phone with `-WaitForPhone`, then orchestrates required-phone preflight, sample generation/transfer, debug APK install/launch, and timestamped report creation for the real Android run, including setup status/setup mode on success and failure-state report creation with the stopping reason when setup stops early and `-SkipReport` was not used.
