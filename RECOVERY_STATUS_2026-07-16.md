...
- Latest validated checkpoint: current `codex/deal-planner-baseline` branch head after pantry OCR hyphenated package-size splitting validation; confirm the exact commit with `git log -1 --oneline`.
- Previous app-code checkpoint before that work: pantry hyphenated package-size parser validation.
