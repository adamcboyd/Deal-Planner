...
- Latest validated checkpoint: current `codex/deal-planner-baseline` branch head after pantry hyphenated package-size validation; confirm the exact commit with `git log -1 --oneline`.
- Previous app-code checkpoint before that work: receipt `x` quantity separator validation.
